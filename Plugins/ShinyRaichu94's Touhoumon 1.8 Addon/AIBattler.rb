...
class Battle::AI::AIBattler
  ...
  def rough_end_of_round_damage
    ...
    if @ai.battle.field.weatherDuration == 1
      ...
      weather = :None if @ai.battle.allBattlers.any? { |b| b.hasActiveAbility?([:CLOUDNINE, :AIRLOCK, :HISOUTEN, :UNCONSCIOUS]) }
  ...
  def effectiveness_of_type_against_battler(type, user = nil, move = nil)
    ...
    return ret if !type
    return ret if type == :EARTH && has_type?(:FLYING) && has_active_item?(:IRONBALL)
    # Get effectivenesses
    if type == :SHADOW
  ...
  def effectiveness_of_type_against_single_battler_type(type, defend_type, user = nil)
    ...
    if !battler.airborne? && defend_type == :FLYING && type == :EARTH
      ret = Effectiveness::NORMAL_EFFECTIVE_MULTIPLIER
    end
    return ret
  end
