...
class PokemonPokedexInfo_Scene
  ...
  def drawPageInfo
    ...
    textpos = [
      ...
      textpos.push([_INTL("Cost:"), 226, 164, :left, base, shadow])
    #end
    if $player.owned?(@species)
      # Write the category
      textpos.push([_INTL("{1} Puppet", species_data.category), 226, 80, :left, base, shadow])
      # Write the height and weight
        ...
        weight = species_data.weight
          ...
          textpos.push([_ISPRINTF("{1:.1f}", weight / 10.0), 286, 164, :left, base, shadow])
          if species_data.abilities.size < 2
            textpos.push([_INTL("{1}", GameData::Ability.get(species_data.abilities[0]).name), 226, 196, :left, base, shadow])
          else
            textpos.push([_INTL("{1}/{2}", GameData::Ability.get(species_data.abilities[0]).name, 
                 GameData::Ability.get(species_data.abilities[1]).name), 226, 196, :left, base, shadow])
          end
        #end
      #end
      if !@show_battled_count
      # Draw the Pokédex entry text
        ...
        textpos.push([_INTL("Base Stats"), 40, 246, :left, base, shadow])
        textpos.push([_INTL("HP: {1}", species_data.base_stats[:HP]), 40, 278, :left, base, shadow])
        textpos.push([_INTL("Attack: {1}", species_data.base_stats[:ATTACK]), 178, 278, :left, base, shadow])
        textpos.push([_INTL("Defense: {1}", species_data.base_stats[:DEFENSE]), 324, 278, :left, base, shadow])
        textpos.push([_INTL("Sp. Atk: {1}", species_data.base_stats[:SPECIAL_ATTACK]), 40, 310, :left, base, shadow])
        textpos.push([_INTL("Sp. Def: {1}", species_data.base_stats[:SPECIAL_DEFENSE]), 178, 310, :left, base, shadow])
        textpos.push([_INTL("Speed: {1}", species_data.base_stats[:SPEED]), 324, 310, :left, base, shadow])
        textpos.push([_INTL("Total: {1}", species_data.base_stats.values.sum), 324, 342, :left, base, shadow])
      end
      # Draw the footprint
      ...
      # Show the owned icon
      imagepos.push(["Graphics/UI/Pokedex/icon_own", 192, 44])
      # Draw the type icon(s)
      species_data.types.each_with_index do |type, i|
        type_number = GameData::Type.get(type).icon_position
        type_rect = Rect.new(0, type_number * 32, 96, 32)
        overlay.blt(226 + (100 * i), 120, @typebitmap.bitmap, type_rect) #296
      end
    else
      # Write the category
      textpos.push([_INTL("????? Puppet"), 226, 80, :left, base, shadow])
      # Write the height and weight
      #if !@show_battled_count
        #if System.user_language[3..4] == "US"   # If the user is in the United States
          ...
          textpos.push([_INTL("????.?"), 286, 164, :left, base, shadow])
          textpos.push([_INTL("?????"), 226, 196, :left, base, shadow])
        #end
      #end
    end
    # Draw all text
