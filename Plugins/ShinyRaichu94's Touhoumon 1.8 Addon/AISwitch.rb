...
class Battle::AI
  ...
  def calculate_entry_hazard_damage(pkmn, side)
    ...
    ret = 0
    # Stealth Rock
    if @battle.sides[side].effects[PBEffects::StealthRock] && GameData::Type.exists?(:EARTH)
      pkmn_types = pkmn.types
      eff = Effectiveness.calculate(:EARTH, *pkmn_types)
...
Battle::AI::Handlers::ShouldSwitch.add(:cure_status_problem_by_switching_out,
  proc { |battler, reserves, ai, battle|
    ...
    if battler.ability == :NATURALCURE || (single_status_cure && single_status_cure == battler.status)
      ...
      if battler.status == :POISON && reserves.none? { |pkmn| pkmn.hasType?(:MIASMA) }
...
Battle::AI::Handlers::ShouldSwitch.add(:yawning,
  proc { |battler, reserves, ai, battle|
    ...
    if battler.ability_active?
      next false if [:INSOMNIA, :NATURALCURE, :REGENERATOR, :SHEDSKIN, :MAINTENANCE].include?(battler.ability_id)
