...
class Battle::AI::AIMove
  ...
  def rough_damage
    ...
    if !@ai.battle.moldBreaker
      ...
      if target.ability_active?
        ...
        when :FIREVEIL
            multipliers[:power_multiplier] /= 2 if calc_type == :WATER   # NOTE: Not calc_type.
        else
    ...
    if @ai.trainer.medium_skill?
      ...
      when :Sandstorm
        if target.has_type?(:BEAST) && specialMove?(calc_type) &&
           function_code != "UseTargetDefenseInsteadOfTargetSpDef"   # Psyshock
          multipliers[:defense_multiplier] *= 1.5
        end
      when :Hail
        if target.has_type?(:ICE) && physicalMove?(calc_type)
          multipliers[:defense_multiplier] *= 1.5
        end
      end
    end
  ...
  def get_score_change_for_additional_effect(user, target = nil)
    ...
    return -999 if target && user.index != target.index &&
                   (target.has_active_ability?(:SHIELDDUST) || target.has_active_ability?(:ADVENT)) && !@ai.battle.moldBreaker
