...
class Battle::AI
  ...
  def pokemon_can_absorb_move?(pkmn, move, move_type)
    ...
    when :LIGHTNINGROD, :MOTORDRIVE, :VOLTABSORB
      return move_type == :WIND
    when :SAPSIPPER
      return move_type == :NATURE
    when :SOUNDPROOF
      move_data = GameData::Move.get(move.id)
  ...
  def pokemon_can_be_poisoned?(pkmn)
    ...
    return false if pkmn.hasType?(:MIASMA)
  ...
  BASE_ABILITY_RATINGS = {
    10 => [:DELTASTREAM, :DESOLATELAND, :HUGEPOWER, :MOODY, :PARENTALBOND,
           ...
           :STANCECHANGE, :WONDERGUARD, :UNZAN, :PLAYGHOST],
    ...
    7  => [:BEASTBOOST, :BULLETPROOF, :COMPOUNDEYES, :DOWNLOAD, :FURCOAT,
           ...
           :VOLTABSORB, :WATERABSORB, :FOCUS, :MAINTENANCE, :WALLOFICE,
           :FIREVEIL, :JEALOUSY],
    6  => [:BATTLEBOND, :CHLOROPHYLL, :COMATOSE, :DARKAURA, :DRYSKIN,
           ...
           :TRACE, :UNAWARE, :VICTORYSTAR, :DOLLWALL],
    5  => [:AFTERMATH, :AIRLOCK, :ANALYTIC, :BERSERK, :BLAZE, :CLOUDNINE,
           ...
           :SWARM, :TANGLINGHAIR, :TORRENT, :UNCONSCIOUS, :HISOUTEN, :INNERPOWER,
           :SPRINGCHARM,:ADVENT],
    4  => [:ANGERPOINT, :BADDREAMS, :CHEEKPOUCH, :CLEARBODY, :CURSEDBODY,
           ...
           :WHITESMOKE, :WONDERSKIN, :MAGICBARRIER, :HAKUREIMIKO, :INFECTIOUS,
           :POISONBODY],
    3  => [:AROMAVEIL, :AURABREAK, :COTTONDOWN, :DAUNTLESSSHIELD,
           ...
           :STICKYHOLD, :SUPERLUCK, :UNNERVE, :WIMPOUT, :HIGHSTRENGTH],
    2  => [:BATTLEARMOR, :COLORCHANGE, :CUTECHARM, :DAMP, :GRASSPELT,
           ...
           :WANDERINGSPIRIT, :WEAKARMOR, :GUARDARMOR, :MYSTERIOUS],
    1  => [:BIGPECKS, :KEENEYE, :MAGMAARMOR, :PICKUP, :RIVALRY, :STENCH, :FRETFUL],
...
Battle::AI::Handlers::AbilityRanking.copy(:HUGEPOWER, :PUREPOWER, :UNZAN)
...
Battle::AI::Handlers::AbilityRanking.add(:SANDFORCE,
  proc { |ability, score, battler, ai|
    next score if battler.has_damaging_move_of_type?(:EARTH, :BEAST, :STEEL)
    next 2
  }
)
...
Battle::AI::Handlers::ItemRanking.addIf(:type_boosting_items,
  proc { |item|
    next [:GOTHICDRESS, :SWIMSUIT, :NINJASUIT, :CAMODRESS, :THICKFUR,
          :CHINADRESS, :NURSEOUTFIT, :GYMTRACKSUIT, :STEWARDESS, :WITCHOUTFIT,
          :BLAZER, :BUNNYSUIT, :KIMONO, :PRIESTESS, :MISTRESS,
          :MAIDOUTFIT, :BRIDALDRESS].include?(item)
  },
  proc { |item, score, battler, ai|
    boosters = {
      :HEART    => [:BLAZER],
