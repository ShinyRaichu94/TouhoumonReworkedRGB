class Battle::Move
  ...
  def pbCalcType(user)
    @powerBoost = false
    ret = pbBaseType(user)
    if ret && GameData::Type.exists?(:WIND)
  ...
  def pbCalcTypeModSingle(moveType, defType, user, target)
    ...
    if !target.airborne? && defType == :FLYING && moveType == :EARTH
      ret = Effectiveness::NORMAL_EFFECTIVE_MULTIPLIER
    end
    return ret
  end
  ...
  def pbCalcTypeMod(moveType, user, target)
    ...
    return ret if moveType == :EARTH && target.pbHasType?(:FLYING) && target.hasActiveItem?(:IRONBALL)
    # Get effectivenesses
    if moveType == :SHADOW
  ...
  def pbCalcDamageMultipliers(user, target, numTargets, type, baseDmg, multipliers)
    ...
    if user.effects[PBEffects::Charge] > 0 && type == :WIND
    ...
    when :Electric
      multipliers[:power_multiplier] *= terrain_multiplier if type == :WIND && user.affectedByTerrain?
    when :Grassy
      multipliers[:power_multiplier] *= terrain_multiplier if type == :NATURE && user.affectedByTerrain?
    when :Psychic
      multipliers[:power_multiplier] *= terrain_multiplier if type == :REASON && user.affectedByTerrain?
    when :Misty
      multipliers[:power_multiplier] /= 2 if type == :FAITH && target.affectedByTerrain?
    end
    # Badge multipliers
    ...
    when :Sandstorm
      if target.pbHasType?(:BEAST) && specialMove? && @function_code != "UseTargetDefenseInsteadOfTargetSpDef"
