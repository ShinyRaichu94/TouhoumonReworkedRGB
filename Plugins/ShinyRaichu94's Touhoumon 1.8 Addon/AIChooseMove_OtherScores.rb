...
#===============================================================================
# Prefer using certain moves for Touhoumon Reworked RGB.
#===============================================================================
#Battle::AI::Handlers::GeneralMoveAgainstTargetScore.add(:Touhoumon_Reworked_AI,
#  proc { |score, move, user, target, ai, battle|
#    if ai.trainer.high_skill?
#      old_score = score
#      score += 5 if move.rough_type == :DREAM && Effectiveness.super_effective?(target.effectiveness_of_type_against_battler(:DREAM, user))
#      score += 5 if move.rough_type == :GHOST && Effectiveness.super_effective?(target.effectiveness_of_type_against_battler(:GHOST, user))
#      score += 5 if move.rough_type == :FLYING && Effectiveness.super_effective?(target.effectiveness_of_type_against_battler(:FLYING, user))
#      score += 5 if move.rough_type == :BEAST && Effectiveness.super_effective?(target.effectiveness_of_type_against_battler(:BEAST, user))
#      score += 5 if move.rough_type == :MIASMA && Effectiveness.super_effective?(target.effectiveness_of_type_against_battler(:MIASMA, user))
#      score += 5 if move.rough_type == :STEEL && Effectiveness.super_effective?(target.effectiveness_of_type_against_battler(:STEEL, user))
#      score += 5 if move.rough_type == :DARK && Effectiveness.super_effective?(target.effectiveness_of_type_against_battler(:DARK, user))
#      score += 5 if move.rough_type == :EARTH && Effectiveness.super_effective?(target.effectiveness_of_type_against_battler(:EARTH, user))
#      score += 5 if move.rough_type == :FIRE && Effectiveness.super_effective?(target.effectiveness_of_type_against_battler(:FIRE, user))
#      score += 5 if move.rough_type == :WATER && Effectiveness.super_effective?(target.effectiveness_of_type_against_battler(:WATER, user))
#      score += 5 if move.rough_type == :WIND && Effectiveness.super_effective?(target.effectiveness_of_type_against_battler(:WIND, user))
#      score += 5 if move.rough_type == :NATURE && Effectiveness.super_effective?(target.effectiveness_of_type_against_battler(:NATURE, user))
#      score += 5 if move.rough_type == :ICE && Effectiveness.super_effective?(target.effectiveness_of_type_against_battler(:ICE, user))
#      score += 5 if move.rough_type == :FAITH && Effectiveness.super_effective?(target.effectiveness_of_type_against_battler(:FAITH, user))
#      score += 5 if move.rough_type == :REASON && Effectiveness.super_effective?(target.effectiveness_of_type_against_battler(:REASON, user))
#      score += 5 if move.rough_type == :HEART && Effectiveness.super_effective?(target.effectiveness_of_type_against_battler(:HEART, user))

#      if ai.trainer.has_skill_flag?("Surge")
#        if target.has_type?(:EARTH)
#          score += 5 if move.rough_type == :WATER && Effectiveness.super_effective?(target.effectiveness_of_type_against_battler(:WATER, user))
#          score += 5 if move.rough_type == :NATURE && Effectiveness.super_effective?(target.effectiveness_of_type_against_battler(:NATURE, user))
#        end
#      end
...
Battle::AI::Handlers::GeneralMoveAgainstTargetScore.add(:target_can_make_moves_Electric_and_be_immune,
  proc { |score, move, user, target, ai, battle|
    ...
    next score if !ai.pokemon_can_absorb_move?(target, move, :WIND) &&
                  !Effectiveness.ineffective?(target.effectiveness_of_type_against_battler(:WIND, user))
