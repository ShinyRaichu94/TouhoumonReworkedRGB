class Battle::Battler
  ...
  def pbTryUseMove(choice, move, specialUsage, skipAccuracyCheck)
    ...
    if @effects[PBEffects::HyperBeam] > 0   # Intentionally before Truant
      ...
      @effects[PBEffects::Truant] = !@effects[PBEffects::Truant] if hasActiveAbility?(:FRETFUL)
      return false
    end
  ...
  def pbSuccessCheckAgainstTarget(move, user, target, targets)
    ...
    if move.damagingMove? && move.calcType == :EARTH &&
    ...
    if move.powderMove?
      #if target.pbHasType?(:NATURE) && Settings::MORE_TYPE_EFFECTS
